...
void buf_push(struct BoundedBuffer* buf, int val){    
    ...
    sem_wait(&buf->capacity); //wait for capacity to be > 0
    pthread_mutex_lock(&buf->mtx); //lock mutex while adding element and posting numElements
...
int buf_pop(struct BoundedBuffer* buf){
    ...
    sem_wait(&buf->numElements); //wait for numElements to be > 0
    pthread_mutex_lock(&buf->mtx); //lock mutex while removing element and posting capacity
